...
<div class="header">
  PHONEME'LE - ${difficulty} - ${guessCount} GUESSES
</div>
...
<p>
Use the phoneme keyboard to make a guess.
</p>
  ...
  for(let row=0;row<maxGuesses;row++){
    ...
    for(let col=0;col<answer.length;col++){
      ...
      if(saved){
        ...
        if(saved.result[col]==='correct'){
          d.classList.add('correct');
        }

        if(saved.result[col]==='close'){
          d.classList.add('close');
        }

        if(saved.result[col]==='wrong'){
...
document.getElementById('enter').onclick=()=>{
  ...
  if(correct){
    ...
    message.textContent =
      'Correct! The English word is ${english.toUpperCase()}.';
  ...
  if(guesses.length>=maxGuesses){
    ...
    message.textContent =
      'No more guesses. The answer was /${symbols.join(" ")}/ (${english.toUpperCase()}).';
...
export function generateWordSearchHtml(
  ...
  const wordSymbols = words.map(
    (word) => word.phonemes
  );

  const englishWords = words.map(
...
<p>
Click the first phoneme and then the last phoneme of a word.
</p>
...
<div
  ...
  class="info"
>
Click the first and last cell of a word.
</div>
